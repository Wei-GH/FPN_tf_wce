...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    if dataset_name not in ['occupation', 'normal', 'pascal', 'coco']:
        raise ValueError('dataSet name must be in pascal, coco occupation and occupation')
    ...
    img_name_batch, img_batch, gtboxes_and_label_batch, num_obs_batch = tf.train.batch(
        ...
        batch_size=batch_size, capacity=1, num_threads=1, dynamic_pad=True)
